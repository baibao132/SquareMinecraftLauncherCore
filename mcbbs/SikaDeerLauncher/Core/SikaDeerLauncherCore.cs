namespace SikaDeerLauncher.Core
{
    ...
    internal sealed class SikaDeerLauncherCore
    {
        ...
        internal bool IMG(string version)
        {
            ...
            if (this.FileExist(string.Concat(textArray1)) == null)
            {
                string str;
                string[] textArray2 = new string[] { Directory.GetCurrentDirectory(), @"\.minecraft\versions\", version, @"\", version, ".jar" };
                if (this.MD5Exists(string.Concat(textArray2)))
                {
                    return false;
                }
                string[] textArray3 = new string[] { Directory.GetCurrentDirectory(), @"\.minecraft\versions\", version, @"\", version, ".jar" };
                if (unzip.UnZipFile(string.Concat(textArray3), @"SikaDeerLauncher\minecraft", out str))
                {
                    this.DeleteFile(@"SikaDeerLauncher\minecraft\assets\minecraft\textures\gui\title\mojang.png");
                    byte[] array = new byte[Resources.mojang1.Length];
                    Resources.mojang1.CopyTo(array, 0);
                    FileStream stream1 = new FileStream(@"SikaDeerLauncher\minecraft\assets\minecraft\textures\gui\title\mojang.png", FileMode.Create, FileAccess.Write);
                    stream1.Write(array, 0, array.Length);
                    stream1.Close();
                    Uzip.Zip(@"SikaDeerLauncher\minecraft\", Directory.GetCurrentDirectory() + @"\.minecraft\versions\" + version + @"\" + version + ".jar", ref str);
                }
                this.DelPathOrFile(@"SikaDeerLauncher\minecraft\");
            }
            return true;
        }
        ...
        {
            new Tools().liteloaderInstall(version);
        }
        ...
        internal bool MD5Exists(string FilePath)
        {
            ...
            string file = this.GetFile(@"SikaDeerLauncher\MD5.Sika");
            if (file != null)
            {
                char[] separator = new char[] { '\n' };
                string[] strArray = file.Split(separator);
                ...
                this.wj(@"SikaDeerLauncher\MD5.Sika", file + "\n" + text);
            }
            else
            {
                this.wj(@"SikaDeerLauncher\MD5.Sika", text);
            }
            return false;
        }
        ...
        {
            new Tools().OptifineInstall(version, patch);
        }
        ...
        internal string uuid(string name)
        {
            ...
            this.SetFile(Directory.GetCurrentDirectory() + @"\SikaDeerLauncher");
            string file = this.GetFile(Directory.GetCurrentDirectory() + @"\SikaDeerLauncher\user.Sika");
            if (file != "")
            {
            ...
            if ((str2 != null) && (str2 != ""))
            {
                ...
                if (file != "")
                {
                    ...
                    this.wj(Directory.GetCurrentDirectory() + @"\SikaDeerLauncher\user.Sika", encrypt.Encrypt(string.Concat(textArray1), "zxttquws"));
                }
                else
                {
                    this.wj(Directory.GetCurrentDirectory() + @"\SikaDeerLauncher\user.Sika", encrypt.Encrypt(name + "|" + root.id, "zxttquws"));
                }
                return root.id;
            }
            str2 = this.token();
            if (file != "")
            {
                ...
                this.wj(Directory.GetCurrentDirectory() + @"\SikaDeerLauncher\user.Sika", encrypt.Encrypt(string.Concat(textArray2), "zxttquws"));
                return str2;
            }
            this.wj(Directory.GetCurrentDirectory() + @"\SikaDeerLauncher\user.Sika", encrypt.Encrypt(name + "|" + str2, "zxttquws"));
            return str2;
        }
